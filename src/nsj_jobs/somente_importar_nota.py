...
class ImportacaoNota(JobCommand):
    ...
    def execute(self, entrada: dict, job, db, log, registro_execucao):
        try:
            ...
            erros_documento = [
                StatusDocumento.sdcErroProcessamento.value,
                StatusDocumento.sdcErroEmissao.value,
                StatusDocumento.sdcErroConsulta.value,
            ]
            documento_situacoes_ok = [
                StatusDocumento.sdcTransmitido,
                StatusDocumento.sdcImportado,
            ]
            documento_situacoes_erro = [
            ...
            for pedido in pedidos:
                ...
                documentos.extend(
                    self.banco.obterDocumentosEnviados(
                ...
                if len(documentos) != 0:
                    ...
                    for documento in documentos:
                        if (
                            int(documento.get("status"))
                            == StatusDocumento.sdcTransmitido.value
                        ):
                            # sucesso! Status = 2
                            ...
                            if (novoStatus is None) or (novoStatus == statusAtual):
                                ...
                                self.banco.registraLog.mensagem(
                                    var_id_pedido,
                                    "Novo Status inválido: " + str(novoStatus),
                                    tipoMsg.inconsistencia,
                                ...
                                t_pedido.camposAtualizar["chave_de_acesso"] = (
                                    documento.get("chave_emissao")
                                )
                                ...
                                t_pedido.camposAtualizar["id_doc_serv_msg"] = (
                                    documento.get("documento")
                                )
                            ...
                            int(documento.get("status"))
                            == StatusDocumento.sdcImportado.value
                            and not pedido["emitir_nota"]
                        ):
                            registro_execucao.informativo(
            ...
            pedidos.extend(
                self.banco.t_pedidos.obterPedidos(
            ...
            for pedido in pedidos:
                if pedido["status"] == Status.Reemitir.value:
                    ...
                    if tentativaAdicionalAtual < self.maximoTentativasConfig:
                        ...
                        proximaTentativa = t_pedido.pedido[
                            "primeira_tentativa"
                        ] + timedelta(
                            minutes=tentativaAdicionalAtual * self.intervalo_tentativas
                        )
                ...
                if int(pedido["status"]) == Status.Cancelamento_Fiscal.value:
                    pathdefault = (
                        path_CancelamentoNFSE
                        if t_pedido.pedido["tipo_nota"] == "NFSE"
                        else path_CancelamentoNFE
                    )
            ...
            if entrada["env"] == "docker":
                ...
                dirInstalacaoERP = (
                    self.banco.dir_instalacao_erp.obterDiretorioInstalacao()
                )
    ...
    def iterarTentativaParaServiceDocument(self, t_pedido: Tpedido, documento):
        ...
        if self.maximoTentativasConfig == 1:
            t_pedido.atualizarTentativa(
                documento.get("datahora_inclusao"), documento.get("datahora_inclusao")
            )
            return False
        ...
        if tentativaAdicionalAtual < self.maximoTentativasConfig:
            ...
            if dataHoraPrimeiraTentativa is None:
                ...
                t_pedido.atualizarTentativa(
                    documento.get("datahora_inclusao"),
                    documento.get("datahora_inclusao"),
                )

            # Calcula a partir de quando deve ser a próxima tentativa
            proximaTentativa = dataHoraPrimeiraTentativa + timedelta(
                minutes=tentativaAdicionalAtual * self.intervalo_tentativas
            )
            if datetime.now() > proximaTentativa:
                t_pedido.atualizarTentativa(
                    dataHoraPrimeiraTentativa,
                    documento.get("datahora_inclusao"),
                    tentativaAdicionalAtual,
                )
    ...
    def validarDados(self, a_pedido: Tpedido):
        ...
        try:
            ...
            if (
                a_pedido.pedido["tipo_nota"] in ["NFE"]
                and a_pedido.pedido["id_operacao"] == ""
            ):
                strAviso = strmsg.format("Código da Operação", "COD_OPERACAO", "vazio")
                erroLista.append(strAviso)

            if a_pedido.pedido["tipo_nota"] in ["NFE"] and a_pedido.pedido[
                "tp_operacao"
            ] not in [item.value for item in Tp_Operacao]:
            ...
            if (
                a_pedido.pedido["tipo_nota"] in ["NFE"]
                and a_pedido.pedido["tipodocumento"] != 55
            ):
                strAviso = strmsg.format(
                    "Tipo de documento",
                    "TIPODOCUMENTO",
                    a_pedido.pedido["tipodocumento"],
                )
                erroLista.append(strAviso)

            if (var_loc_estoq != "") and var_loc_estoq is not None:
                if a_pedido.pedido["tipo_nota"] in [
                    "NFE"
                ] and not self.banco.localEstoqueValido(var_loc_estoq, var_id_Estab):
                    strAviso = strmsg.format(
            ...
            if a_pedido.lstCliente is None or len(a_pedido.lstCliente) == 0:
                erroLista.append(
                    "Cliente não encontrado com o CNPJ/CPF informado: "
                    + str(a_pedido.pedido["cnpj_cliente"])
                )
            ...
            if a_pedido.pedido["tipo_nota"] in ["NFE"]:
                for produto in a_pedido.lstProdutos:
                    if (produto.get("sem_saldo") == 1) and (
                        produto.get("mensagem_erro") != ""
                    ):
                    ...
                    if produto.get("prod_nao_existe") == 1:
                        strAviso = strmsg.format(
                            "Código do Produto",
                            "COD_PRODUTO",
                            produto.get("cod_produto"),
                        )
                        erroLista.append(strAviso)

                    if "USO_CONSUMO" in produto.get("cod_produto"):
                        erroLista.append(
                            f'Produto {produto.get("cod_produto")} de uso e consumo não pode fazer parte do pedido.'
                        )

                    var_loc_estoq = str(produto.get("localestoque"))

                    if (var_loc_estoq != "") and (var_loc_estoq is not None):
                        if not self.banco.localEstoqueValido(
                            var_loc_estoq, var_id_Estab
                        ):
                            erroLista.append(
                    ...
                    if (produto.get("documentoreferenciado_chave") is not None) and (
                        produto.get("documentoreferenciado_chave") != ""
                    ):
            ...
            for pagamento in a_pedido.lstFormaPagamento:
                if (pagamento.get("tipoformapagamento") is None) or (
                    pagamento.get("tipoformapagamento") == ""
                ):
                    strAviso = strmsg.format(
                        "Tipo de Pagamento",
                        "TIPOFORMAPAGAMENTO",
            ...
            if len(erroLista) == 0:
                ...
                self.listarErros(
                    strPedido, erroLista, tipoMsg.inconsistencia, num_pedido
                )
    ...
    def novoStatus(self, status_Doc: int, statusAtualPedido: int):
        ...
        if statusAtualPedido in [
            Status.Rejeitado.value,
            Status.Aberto.value,
            Status.Reemitir.value,
            Status.Cancelamento_Fiscal.value,
        ]:
